...
io.on('connection', socket => {
  ...
  socket.on('userJoined', (data, cb) => {
    ...
    socket.broadcast
      .to(data.room)
      .emit('newMessage', message('admin', `User ${data.name} connected.`))
  })
  ...
  socket.on('leftChat', (id, cb) => {
    const user = users.remove(id);

    if (user) {
      io.to(user.room).emit('updateUsers', users.getByRoom(user.room))
      io.to(user.room).emit('newMessage', message('admin', `User ${user.name} left!`));
    }
